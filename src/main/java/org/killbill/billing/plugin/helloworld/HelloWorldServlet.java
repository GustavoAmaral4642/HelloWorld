...
public class HelloWorldServlet {
    ...
    public void hello(@Local @Named("killbill_tenant") final Optional<Tenant> tenant) {
        ...
        if(tenant != null && tenant.isPresent() ) {
            ...
            logger.info("=== Hello World Plugin chamado as: {} ===", System.currentTimeMillis());
            logger.info("Request recebida para tenant: {}", tenant.isPresent() ? tenant.get().getId() : "NO_TENANT");
            logger.info("Total de requisições recebidas: {}", requestCount);
        }
        else {
